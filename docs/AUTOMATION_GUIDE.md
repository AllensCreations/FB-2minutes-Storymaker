...
Make a standard HTTP POST request from Google Flow / Google Apps Script:
...
Once GitHub Actions completes the FFmpeg rendering (usually 60-90 seconds), it automatically pings your `make_webhook_url` with the exact minimal payload:
...
2. Add:
   * `GITHUB_TOKEN` (or `GH_PAT`): A GitHub Personal Access Token with `repo` permissions.
